...
function formatFetchError(error: unknown): string {
	...
	if (cause instanceof Error) {
		...
		const codeStr = causeWithCode.code ? ` (${causeWithCode.code})` : "";
		return `${error.message}: ${cause.message}${codeStr}`;
	}
...
async function fetchJson(url: string, init: RequestInit): Promise<unknown> {
	...
	try {
		...
		if (proxyUrl) {
			...
			response = await undiciFetch(url, { ...init, dispatcher } as Parameters<typeof undiciFetch>[1]);
		} else {
