...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: free-code login <command> [provider]
...
Examples:
  free-code login              # interactive provider selection
  free-code login anthropic    # login to specific provider
  free-code list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'free-code list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'free-code --help' for usage`);
	process.exit(1);
}
