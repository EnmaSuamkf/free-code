...
```bash
npm install @free/pi-ai
```

TypeBox exports are re-exported from `@free/pi-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@free/pi-ai';
...
```typescript
import { Type, Tool, StringEnum } from '@free/pi-ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@free/pi-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@free/pi-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@free/pi-ai';
...
```typescript
import { getModel, stream } from '@free/pi-ai';
...
```typescript
import { getProviders, getModels, getModel } from '@free/pi-ai';
...
```typescript
import { Model, stream } from '@free/pi-ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@free/pi-ai';
...
```typescript
import { getModel, complete, Context } from '@free/pi-ai';
...
```typescript
import { Context, getModel, complete } from '@free/pi-ai';
...
- OAuth login flows are not supported in browser environments. Use the `@free/pi-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@free/pi-ai';
...
```bash
npx @free/pi-ai login              # interactive provider selection
npx @free/pi-ai login anthropic    # login to specific provider
npx @free/pi-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@free/pi-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@free/pi-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@free/pi-ai/oauth';
...
```typescript
import { getModel, complete } from '@free/pi-ai';
import { getOAuthApiKey } from '@free/pi-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@free/pi-ai`
