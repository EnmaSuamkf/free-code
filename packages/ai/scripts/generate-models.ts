...
const GITHUB_COPILOT_ENABLED_MODEL_IDS = new Set([
	"claude-sonnet-4",
	"claude-sonnet-4.5",
	"claude-haiku-4.5",
	"claude-opus-4.5",
	...
	try {
		...
		if (data["github-copilot"]?.models) {
			for (const [modelId, model] of Object.entries(data["github-copilot"].models)) {
				...
				if (m.tool_call !== true) continue;
				if (m.status === "deprecated") continue;
				if (!GITHUB_COPILOT_ENABLED_MODEL_IDS.has(modelId)) continue;
...
async function generateModels() {
	...
	// Add missing GitHub Copilot models until models.dev includes them.
	...
	const copilotNanoBaseModel = allModels.find(
		(m) => m.provider === "github-copilot" && m.id === "gpt-5.4-mini",
	);
	if (copilotNanoBaseModel) {
		if (!allModels.some((m) => m.provider === "github-copilot" && m.id === "gpt-5.4-nano")) {
			allModels.push({
				...copilotNanoBaseModel,
				id: "gpt-5.4-nano",
				name: "GPT-5.4 Nano",
				input: ["text"],
			});
