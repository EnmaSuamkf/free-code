...
- Vertex (ADC): do not cache a negative "credentials file missing" result so `gcloud auth application-default login` after the first model list is detected without restarting the process.
...
- Lazy-load built-in provider modules and root provider wrappers so importing `@free/pi-ai` no longer eagerly loads provider SDKs, significantly reducing base startup cost without changing dependency installation footprint ([#2297](https://github.com/badlogic/pi-mono/issues/2297))
...
- Fixed `@free/pi-ai/oauth` package exports to point directly at built `dist` files, avoiding broken TypeScript resolution through unpublished wrapper targets ([#1856](https://github.com/badlogic/pi-mono/issues/1856)).
...
- Moved Node OAuth runtime exports off the top-level package entry. Import OAuth login/refresh functions from `@free/pi-ai/oauth` instead of `@free/pi-ai` ([#1814](https://github.com/badlogic/pi-mono/issues/1814))
...
- Re-exported TypeBox `Type`, `Static`, and `TSchema` from `@free/pi-ai` to match documentation and avoid duplicate TypeBox type identity issues in pnpm setups ([#1338](https://github.com/badlogic/pi-mono/issues/1338))
...
- **Agent API moved**: All agent functionality (`agentLoop`, `agentLoopContinue`, `AgentContext`, `AgentEvent`, `AgentTool`, `AgentToolResult`, etc.) has moved to `@free/pi-agent-core`. Import from that package instead of `@free/pi-ai`.
