...
- run interactive coding agent session: `npx @free/pi-coding-agent`
- enter `/login` command
...
Update mom anytime with `npm install -g @free/pi-mom`. This only updates the Node.js app on your host. Anything mom installed inside the Docker container remains unchanged.
