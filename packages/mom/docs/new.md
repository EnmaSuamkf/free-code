...
```typescript
import { type AgentSessionEvent } from "@free/pi-coding-agent";
...
import type { MomCustomTool, ToolAPI } from "@free/pi-mom";
...
Tools are loaded via jiti. They can import any 3rd party dependencies (install in the tool directory). Imports of `@free/pi-ai` and `@free/pi-mom` are aliased to the running mom bundle.
