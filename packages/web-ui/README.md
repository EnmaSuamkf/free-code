...
Reusable web UI components for building AI chat interfaces powered by [@free/pi-ai](../ai) and [@free/pi-agent-core](../agent).
...
```bash
npm install @free/pi-web-ui @free/pi-agent-core @free/pi-ai
```
...
```typescript
import { Agent } from '@free/pi-agent-core';
import { getModel } from '@free/pi-ai';
import {
  ChatPanel,
  AppStorage,
...
```typescript
import { defaultConvertToLlm, convertAttachments } from '@free/pi-web-ui';
...
```typescript
import { createJavaScriptReplTool } from '@free/pi-web-ui';
...
```typescript
import { createExtractDocumentTool } from '@free/pi-web-ui';
...
```typescript
import { registerToolRenderer, type ToolRenderer } from '@free/pi-web-ui';
...
```typescript
import { loadAttachment, type Attachment } from '@free/pi-web-ui';
...
```typescript
import { createStreamFn, shouldUseProxyForProvider, isCorsError } from '@free/pi-web-ui';
...
```typescript
import { SettingsDialog, ProvidersModelsTab, ProxyTab, ApiKeysTab } from '@free/pi-web-ui';
...
```typescript
import { SessionListDialog } from '@free/pi-web-ui';
...
```typescript
import { ApiKeyPromptDialog } from '@free/pi-web-ui';
...
```typescript
import { ModelSelector } from '@free/pi-web-ui';
...
```css
@import '@free/mini-lit/themes/claude.css';
@tailwind base;
@tailwind components;
@tailwind utilities;
```
...
```typescript
import { i18n, setLanguage, translations } from '@free/pi-web-ui';
