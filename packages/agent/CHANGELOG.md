...
- Mid-turn session updates (e.g. `set_active_profile`, `set_active_model`): the agent loop froze `AgentContext` and `AgentLoopConfig` at continuation start, so the next LLM request in the same user turn still used the previous tools, system prompt, and model. The loop now calls optional `refreshRuntimeContext` before each provider stream; `Agent` implements it by copying live `state.tools`, `state.systemPrompt`, and `state.model`.
...
- **Agent loop moved from `@free/pi-ai`**: The `agentLoop`, `agentLoopContinue`, and related types have moved to this package. Import from `@free/pi-agent-core` instead.
