...
Stateful agent with tool execution and event streaming. Built on `@free/pi-ai`.

## Installation

```bash
npm install @free/pi-agent-core
```
...
```typescript
import { Agent, streamProxy } from "@free/pi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@free/pi-agent-core";
