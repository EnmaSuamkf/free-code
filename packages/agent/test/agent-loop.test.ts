...
describe("agentLoop with AgentMessage", () => {
	...
	it("refreshRuntimeContext runs before each LLM call so mid-turn state can update tools/prompt/model", async () => {
		const toolSchema = Type.Object({ value: Type.String() });
		let livePrompt = "prompt-a";
		const tool: AgentTool<typeof toolSchema, { value: string }> = {
			name: "switchPrompt",
			label: "Switch",
			description: "Switch live prompt",
			parameters: toolSchema,
		...
		const userPrompt: AgentMessage = createUserMessage("run tool");
		...
		const config: AgentLoopConfig = {
			...
			refreshRuntimeContext: (ctx) => {
				ctx.systemPrompt = livePrompt;
			},
		};

		const streamFn = (_model: Model<any>, llmContext: Context) => {
			promptsSeen.push(llmContext.systemPrompt ?? "");
			const stream = new MockAssistantStream();
			queueMicrotask(() => {
				if (callIndex === 0) {
					const message = createAssistantMessage(
						[{ type: "toolCall", id: "t1", name: "switchPrompt", arguments: { value: "x" } }],
						"toolUse",
					);
					stream.push({ type: "done", reason: "toolUse", message });
				} else {
		...
		expect(promptsSeen).toEqual(["prompt-a", "prompt-b"]);
	});
