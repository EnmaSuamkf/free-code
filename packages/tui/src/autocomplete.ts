...
export class CombinedAutocompleteProvider implements AutocompleteProvider {
	...
	async getSuggestions(
		...
		if (textBeforeCursor.startsWith("/")) {
			...
			if (spaceIndex === -1) {
				// Command name completion — only in non-forced mode (typing, not Tab)
				if (options.force) {
					// fall through to regular path completion
				} else {
				...
				// Argument completion — available in both regular typing and forced Tab
				const commandName = textBeforeCursor.slice(1, spaceIndex);
				const argumentText = textBeforeCursor.slice(spaceIndex + 1);
				const canonical = commandName.replace(/:\d+$/, "");
				const matchingCommands = this.commands.filter((cmd) => {
					if (!("name" in cmd) || !cmd.getArgumentCompletions) return false;
					if (cmd.name === commandName) return true;
					return cmd.name.replace(/:\d+$/, "") === canonical;
				});
				const merged: AutocompleteItem[] = [];
				const seen = new Set<string>();
				for (const cmd of matchingCommands) {
					if (!("name" in cmd) || !cmd.getArgumentCompletions) continue;
					const argumentSuggestions = await cmd.getArgumentCompletions(argumentText);
					if (!Array.isArray(argumentSuggestions)) continue;
					for (const item of argumentSuggestions) {
				...
				// No argument completions — suppress path completion for slash commands
				// to avoid false positives from the generic path extractor
				if (!options.force) {
					return null;
				}
			}
	...
	applyCompletion(
		...
		if (isSlashCommand) {
			...
			return {
				lines: newLines,
				cursorLine,
				cursorCol: beforePrefix.length + item.value.length + 1, // +1 for "/"
			};
		}
