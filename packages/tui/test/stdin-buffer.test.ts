...
describe("StdinBuffer", () => {
	...
	describe("Regular Characters", () => {
		...
		it("should handle unicode characters as a burst paste", () => {
			processInput("hello 世界");
			// 8 printable chars in a single chunk → emitted as a synthetic paste so consumers
			// can detect bracketed-paste-less drops (e.g. macOS Terminal.app drag-and-drop).
			assert.deepStrictEqual(emittedSequences, []);
			assert.deepStrictEqual(emittedPastes, ["hello 世界"]);
		});

		it("should treat runs of 4+ printable characters in a single chunk as a paste", () => {
			processInput("abcd");
			assert.deepStrictEqual(emittedSequences, []);
			assert.deepStrictEqual(emittedPastes, ["abcd"]);
		});

		it("should treat a drop-like quoted file path as a paste even without bracketed markers", () => {
			processInput("'/Users/foo/bar.png'");
			assert.deepStrictEqual(emittedSequences, []);
			assert.deepStrictEqual(emittedPastes, ["'/Users/foo/bar.png'"]);
		});
	});
