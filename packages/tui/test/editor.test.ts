...
describe("Editor component", () => {
	...
	describe("File drop chips", () => {
		const absPath = "/Users/dev/Documents/hola-mundo.png";
		const basename = "hola-mundo.png";

		function dropFile(editor: Editor, pasted: string): string {
			editor.handleInput(`\x1b[200~${pasted}\x1b[201~`);
			return editor.getText();
		}

		it("converts a single absolute path drop into a [file #N basename] chip", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const text = dropFile(editor, absPath);
			assert.match(text, /^\[file #1 hola-mundo\.png\]$/);
		});

		it("converts a single-quoted absolute path drop into a chip", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const text = dropFile(editor, `'${absPath}'`);
			assert.match(text, /^\[file #1 hola-mundo\.png\]$/);
		});

		it("converts a double-quoted path with escaped quotes into a chip", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const text = dropFile(editor, `"/tmp/a \\"b\\".png"`);
			assert.match(text, /^\[file #1 a "b"\.png\]$/);
		});

		it("converts a backslash-escaped-space path into a chip", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const text = dropFile(editor, "/tmp/My\\ Folder/img.png");
			assert.match(text, /^\[file #1 img\.png\]$/);
		});

		it("converts a file:// URI into a chip and strips the scheme in the hyperlink target", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const text = dropFile(editor, "file:///tmp/foo%20bar/baz.png");
			assert.match(text, /^\[file #1 baz\.png\]$/);
		});

		it("creates one chip per file on a multi-file drop (space-separated)", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const text = dropFile(editor, `'/tmp/a.png' '/tmp/b.png'`);
			assert.match(text, /^\[file #1 a\.png\] \[file #2 b\.png\]$/);
		});

		it("does not chip when the payload contains non-path tokens (falls back to normal paste)", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const text = dropFile(editor, "not a path /also/ok");
			assert.strictEqual(text, "not a path /also/ok");
		});

		it("does not chip multi-line paste that happens to start with a path", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const text = dropFile(editor, `${absPath}\nmore text`);
			assert.ok(!/\[file #/.test(text), `expected no file chip, got ${JSON.stringify(text)}`);
		});

		it("chips a drop even when the terminal appends a trailing newline (Terminal.app / iTerm2 behavior)", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const text = dropFile(editor, `'${absPath}'\n`);
			assert.match(text, /^\[file #1 hola-mundo\.png\]$/);
		});

		it("chips a drop with surrounding whitespace and CRLF line ending", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const text = dropFile(editor, `  ${absPath}  \r\n`);
			assert.match(text, /^\[file #1 hola-mundo\.png\]$/);
		});

		it("still chips the path even when the file does not exist on disk", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const text = dropFile(editor, "/definitely/not/a/real/path-xyz/ghost.bin");
			assert.match(text, /^\[file #1 ghost\.bin\]$/);
		});

		it("treats the chip as a single unit for cursor movement", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			// Drop first so there is no word char before the chip (otherwise a readability space
			// is prepended — covered in its own test below).
			dropFile(editor, absPath);
			editor.handleInput("B");

			editor.handleInput("\x01"); // Ctrl+A: home
			assert.deepStrictEqual(editor.getCursor(), { line: 0, col: 0 });

			const chip = editor.getText().match(/\[file #\d+ [^\]]+\]/)![0];
			editor.handleInput("\x1b[C"); // should skip entire chip
			assert.deepStrictEqual(editor.getCursor(), { line: 0, col: chip.length });
		});

		it("deletes the chip atomically with a single backspace", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			dropFile(editor, absPath);
			editor.handleInput("B");
			editor.handleInput("\x1b[D"); // past B
			editor.handleInput("\x7f"); // backspace: removes the chip
			assert.strictEqual(editor.getText(), "B");
		});

		it("prepends a readability space when dropping directly after a word character", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			editor.handleInput("hola");
			dropFile(editor, absPath);
			assert.match(editor.getText(), /^hola \[file #1 hola-mundo\.png\]$/);
		});

		it("expands file chip back to the original raw text on submit", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			let submitted = "";
			editor.onSubmit = (text) => {
				submitted = text;
			...
			dropFile(editor, quoted);
			assert.match(editor.getText(), /^\[file #1 hola-mundo\.png\]$/);
			editor.handleInput("\r");
			assert.strictEqual(submitted, quoted);
		});

		it("expands multi-drop chips back to their original tokens separated by spaces", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			let submitted = "";
			editor.onSubmit = (text) => {
				submitted = text;
			};
			dropFile(editor, `'/tmp/a.png' '/tmp/b.png'`);
			editor.handleInput("\r");
			assert.strictEqual(submitted, `'/tmp/a.png' '/tmp/b.png'`);
		});

		it("renders the chip as [basename] wrapped in an OSC 8 hyperlink", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			dropFile(editor, absPath);
			const rendered = editor.render(80).join("\n");
			assert.ok(
				rendered.includes(`\x1b]8;;file://${absPath}\x07[${basename}]\x1b]8;;\x07`),
				`rendered output missing OSC 8 chip: ${JSON.stringify(rendered)}`,
			);
			const visible = stripVTControlCharacters(rendered);
			assert.ok(
				visible.includes(`[${basename}]`),
				`visible output missing [${basename}]: ${JSON.stringify(visible)}`,
			);
			assert.ok(
				!visible.includes("[file #"),
				`visible output should not contain the raw marker id: ${JSON.stringify(visible)}`,
			);
		});

		it("does not treat manually typed `[file #N ...]` text as atomic (no valid id)", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const fakeMarker = "[file #99 fake.txt]";
			for (const ch of fakeMarker) editor.handleInput(ch);
			assert.strictEqual(editor.getText(), fakeMarker);
			editor.handleInput("\x01"); // Ctrl+A
			editor.handleInput("\x1b[C"); // Right
