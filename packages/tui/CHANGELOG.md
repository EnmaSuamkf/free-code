...
- Editor now detects drag-and-drop file paste payloads (absolute, `~/`, `./`, `../`, `file://`, single/double quoted, or backslash-escaped) and replaces them with compact `[file #N basename]` chips rendered as OSC 8 hyperlinks to `file://` so supporting terminals (iTerm2, WezTerm, Ghostty, Terminal.app, VSCode) make them clickable. Chips behave as atomic graphemes for cursor movement, backspace, and word-wrap, and expand back to the original shell-equivalent text on submit. Multi-file drops create one chip per file.
- `StdinBuffer` now emits runs of 4+ printable characters arriving in a single stdin chunk as a synthetic `paste` event instead of per-character `data` events. This lets terminals that don't wrap drag-and-drop in bracketed paste mode (notably macOS Terminal.app) still deliver drops to consumers as paste, enabling features like the editor file-drop chips. Regular typing and typematic key repeat are unaffected because each keystroke arrives as its own stdin read.

### Changed

- Default `tui.input.newLine` now includes `ctrl+j` in addition to `shift+enter`, for terminals that do not send a distinct Shift+Enter sequence. Documentation (for example `packages/coding-agent/docs/terminal-setup.md` and fork `docs/free-code-local-setup.md`) now states that VS Code / Cursor `workbench.action.terminal.sendSequence` must use **`\u001b[13;2u`** in `args.text` for Shift+Enter, not **`\u001b\r`**, which does not encode that key for the TUI.

### Fixed

- `Markdown.invalidate()` now clears the cached default style prefix so theme changes can re-render markdown with updated default text/background ANSI sequences.
