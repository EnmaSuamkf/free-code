...
import { CONFIG_DIR_NAME, ENV_AGENT_PREFIX, getAgentDir, getCliResourceFallbackRoot } from "../config.js";
...
export interface ResourceLoader {
	...
	getAgentResources(): Array<{ path: string; sourceInfo?: SourceInfo }>;
...
function loadContextFilesFromDir(dir: string): Array<{ path: string; content: string }> {
	// Priority: FREE_CODE.md > EDO.md (legacy) > CLAUDE.md > AGENT.md / AGENTS.md.
	const primaryContext = existsSync(join(dir, "FREE_CODE.md")) ? "FREE_CODE.md" : "EDO.md";
	const candidates = [primaryContext, "CLAUDE.md", "AGENTS.md", "AGENT.md"];
...
function loadProjectContextFiles(
	...
	// Priority: Project-specific (FREE_CODE.md > EDO.md legacy > CLAUDE.md > AGENT.md) > Global > Ancestors
	// First, load all from the current working directory (project root)
	const projectContexts = loadContextFilesFromDir(resolvedCwd);
	for (const file of projectContexts) {
	...
	// Load ancestor configurations (excluding cwd which we already tried)
	const ancestorContextFiles: Array<{ path: string; content: string }> = [];

	let currentDir = resolve(resolvedCwd, ".."); // Start from parent directory
	const root = resolve("/");

	while (true) {
		const ancestorContexts = loadContextFilesFromDir(currentDir);
		for (const file of ancestorContexts) {
			if (!seenPaths.has(file.path)) {
...
export interface DefaultResourceLoaderOptions {
	...
	/** @deprecated No longer honored — context files (FREE_CODE.md / EDO.md / CLAUDE.md / AGENTS.md) are always loaded. Accepted only so existing callers keep compiling. */
	noAgentsFiles?: boolean;
...
export class DefaultResourceLoader implements ResourceLoader {
	...
	private agentResources: Array<{ path: string; sourceInfo?: SourceInfo }>;
	...
	async reload(): Promise<void> {
		...
		const enabledAgentResources = getEnabledResources(resolvedPaths.agents);
		...
		for (const p of this.additionalExtensionPaths) {
			if (isLocalPath(p) && !existsSync(p)) {
				const rootHint =
					getCliResourceFallbackRoot() === undefined
						? ` Set ${ENV_AGENT_PREFIX}_ROOT to your monorepo checkout (e.g. export ${ENV_AGENT_PREFIX}_ROOT=/path/to/free-code) so relative -e paths resolve when your shell cwd is another project.`
						: "";
				extensionsResult.errors.push({ path: p, error: `Extension path does not exist: ${p}.${rootHint}` });
			}
		}
		...
		// Context files (FREE_CODE.md > EDO.md legacy > CLAUDE.md > AGENTS.md, cwd > global > ancestors) are ALWAYS loaded.
		// This is intentionally not gated by any flag/setting so plugin and FreeCodeMac behave like the CLI.
		const agentsFiles = {
			agentsFiles: loadProjectContextFiles({ cwd: this.cwd, agentDir: this.agentDir }),
		};

		// Load enabled agent .md files and merge into context files
		const agentContextFiles = this.loadAgentFiles(enabledAgentResources, metadataByPath);
		this.agentResources = agentContextFiles.map((f) => ({ path: f.path, sourceInfo: f.sourceInfo }));
		agentsFiles.agentsFiles.push(...agentContextFiles);
	...
	private loadAgentFiles(
		agentResources: Array<{ path: string; enabled: boolean; metadata: PathMetadata }>,
		metadataByPath: Map<string, PathMetadata>,
	): Array<{ path: string; content: string; sourceInfo?: SourceInfo }> {
		const files: Array<{ path: string; content: string; sourceInfo?: SourceInfo }> = [];
		for (const resource of agentResources) {
			try {
				const content = readFileSync(resource.path, "utf-8");
				const sourceInfo =
					this.findSourceInfoForPath(resource.path, undefined, metadataByPath) ??
					this.getDefaultSourceInfoForPath(resource.path);
				files.push({ path: resource.path, content, sourceInfo });
			} catch {
	...
	private getDefaultSourceInfoForPath(filePath: string): SourceInfo {
		...
		const projectRoots = [
			...
			join(this.cwd, CONFIG_DIR_NAME, "agent", "extensions"),
		];
