...
export interface WriteToolDetails {
	/** Unified diff of the changes made (only when overwriting an existing file) */
	diff: string;
	...
	return {
				...
				() =>
					new Promise<{ content: Array<{ type: "text"; text: string }>; details: WriteToolDetails | undefined }>(
						(resolve, reject) => {
							...
							(async () => {
								try {
									// Read existing content before overwriting (for diff).
									...
									// Generate diff if the file previously existed.
									let details: WriteToolDetails | undefined;
									if (previousContent !== null) {
										const oldNormalized = normalizeToLF(stripBom(previousContent).text);
										const newNormalized = normalizeToLF(stripBom(content).text);
										if (oldNormalized !== newNormalized) {
											const diffResult = generateDiffString(oldNormalized, newNormalized);
		...
		renderResult(result, _options, theme, context) {
			const output = formatWriteResult(
				...
				{ ...result, isError: context.isError } as any,
				theme,
			);
			if (!output) {
