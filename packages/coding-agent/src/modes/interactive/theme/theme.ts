...
import type { EditorTheme, MarkdownTheme, SelectListTheme } from "@free/pi-tui";
import { type Static, Type } from "@sinclair/typebox";
import { TypeCompiler } from "@sinclair/typebox/compiler";
import chalk from "chalk";
import { highlight, supportsLanguage } from "cli-highlight";
import { getCustomThemesDir, getThemesDir, syncBundledThemesFromPackage } from "../../../config.js";
...
/** JSON schema file lives next to theme files; it is not a loadable theme. */
const BUILTIN_THEME_DIR_SCHEMA_FILE = "theme-schema.json";

function getBuiltinThemes(): Record<string, ThemeJson> {
	if (!BUILTIN_THEMES) {
		BUILTIN_THEMES = loadBuiltinThemesOnce();
	}
...
function parseThemeJson(label: string, json: unknown): ThemeJson {
	if (!validateThemeJson.Check(json)) {
		...
		if (missingColors.length > 0) {
			...
			errorMessage += "\nSee the built-in theme JSON files in the package theme directory for reference values.";
		}
...
function loadBuiltinThemesOnce(): Record<string, ThemeJson> {
	...
	for (const file of fs.readdirSync(themesDir).sort()) {
		if (!file.endsWith(".json") || file === BUILTIN_THEME_DIR_SCHEMA_FILE) {
			continue;
		}
		const name = file.slice(0, -5);
		const filePath = path.join(themesDir, file);
		const content = fs.readFileSync(filePath, "utf-8");
		result[name] = parseThemeJsonContent(filePath, content);
	}
	return result;
}
...
const THEME_KEY = Symbol.for("@free/pi-coding-agent:theme");
...
export function getSettingsListTheme(): import("@free/pi-tui").SettingsListTheme {
	return {
