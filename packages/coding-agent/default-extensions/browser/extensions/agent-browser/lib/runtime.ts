...
export function shouldAppendBrowserSystemPrompt(prompt: string): boolean {
	...
	// VS Code/Cursor extension CDP attach recipe (and similar): already prescribes agent_browser + --cdp JSON.
	if (
		normalizedPrompt.startsWith("Execute agent_browser with exactly these params:") &&
		normalizedPrompt.includes('"--cdp"')
	) {
		return false;
	}
