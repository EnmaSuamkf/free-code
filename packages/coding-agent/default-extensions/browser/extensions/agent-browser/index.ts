...
import { isToolCallEventType, type ExtensionAPI } from "@free/pi-coding-agent";
...
function extractCdpPort(args: string[]): number | undefined {
	...
	try {
		const port = parseInt(new URL(args[idx + 1]).port, 10);
...
export default function agentBrowserExtension(pi: ExtensionAPI) {
	...
	let sessionCdpPorts = new Map<string, number>();
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		sessionCdpPorts = new Map<string, number>();
	});
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			const cdpPort = extractCdpPort(params.args);
			...
			try {
				...
				if (executionPlan.sessionName) {
					if (executionPlan.commandInfo.command === "close" && succeeded) {
						...
						const portToKill = sessionCdpPorts.get(executionPlan.sessionName);
						if (portToKill) {
							sessionCdpPorts.delete(executionPlan.sessionName);
							exec(`kill $(lsof -ti tcp:${portToKill}) 2>/dev/null`, () => undefined);
						}
					} else if (currentSessionTabTarget) {
						sessionTabTargets.set(executionPlan.sessionName, currentSessionTabTarget);
					}
					if (cdpPort && succeeded) {
						sessionCdpPorts.set(executionPlan.sessionName, cdpPort);
					}
				}
