# free-code-agent-browser

Dedicated `free-code` integration of `agent-browser` as a native tool.
...
- Thoroughly check official `free-code` docs/examples/source behavior before inventing bespoke integration patterns. Prefer an official `free-code` mechanism whenever one exists.
...
Use an end-to-end interactive `free-code` run inside `tmux`.

### Rules

- For local checkout validation, launch `free-code --no-extensions -e .` from this repository root so only the checkout copy loads, even if the package is installed globally.
- For code changes, isolated `free-code` agent sessions that load this local checkout are a pre-commit requirement; validate behavior there before commit/push.
- Pre-commit validation should exercise the local checkout code, because those changes are not published yet.
- For post-publish installed-package validation, update the real globally installed package and run plain `free-code` so verification exercises the published artifact rather than the local checkout.
- For installed-package validation, run plain `free-code` and confirm the globally installed package loads without any repo-local `.free-code/extensions/` shim.
- Use `tmux` via bash commands.
- Do **not** use the free-code interactive shell extension for this workflow.
- Drive `free-code` like a real user by sending prompts normally.
- Do **not** delegate testing or review to other agents or isolated `free-code` sessions for this repository.
- After extension changes, `/reload` is the minimum, but a full close-and-relaunch of `free-code` is preferred for higher confidence.
...
- native tool invocation works from interactive `free-code`
- `open` works
- `snapshot -i` works
- `click` works
...
- full restart + `/resume` free-codecks up changed extension code
