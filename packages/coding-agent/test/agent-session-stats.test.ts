...
import { type AssistantMessage, getModel, type Usage } from "@free/pi-ai";
...
function createUsageParts(input: number, output: number, cacheRead: number, cacheWrite: number): Usage {
	return {
		input,
		output,
		cacheRead,
...
describe("AgentSession.getSessionStats", () => {
	it("reports token total as input plus output only", () => {
		const { session, sessionManager } = createSession();

		try {
			sessionManager.appendMessage(createUserMessage("hello", 1));
			sessionManager.appendMessage(createAssistantMessageWithUsage("hi", createUsageParts(100, 25, 900, 50), 2));
			syncAgentMessages(session, sessionManager);

			const stats = session.getSessionStats();
			expect(stats.tokens.input).toBe(100);
			expect(stats.tokens.output).toBe(25);
			expect(stats.tokens.cacheRead).toBe(900);
			expect(stats.tokens.cacheWrite).toBe(50);
			expect(stats.tokens.total).toBe(125);
		} finally {
			session.dispose();
		}
	});
...
describe("AgentSession RPC built-in /session", () => {
	it("emits session_info when prompt is exactly /session with source rpc", async () => {
		const { session } = createSession();
		const events: AgentSessionEvent[] = [];
		const unsub = session.subscribe((e) => events.push(e));

		try {
			await session.prompt("/session", { source: "rpc" });
			const info = events.filter(
				(e): e is Extract<AgentSessionEvent, { type: "session_info" }> => e.type === "session_info",
			);
			expect(info).toHaveLength(1);
			expect(info[0].stats.sessionId).toBeDefined();
			expect(info[0].stats.totalMessages).toBe(0);
		} finally {
			unsub();
			session.dispose();
		}
