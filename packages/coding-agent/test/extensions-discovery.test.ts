...
describe("extensions discovery", () => {
	...
	it("skips top-level _*.ts and _*.js (shared modules, not extension entry points)", async () => {
		fs.writeFileSync(path.join(extensionsDir, "real.ts"), extensionCode);
		fs.writeFileSync(path.join(extensionsDir, "_themeMap.ts"), `export const helper = 1;\n`);
		fs.writeFileSync(path.join(extensionsDir, "_helper.js"), `export const x = 1;\n`);

		const result = await discoverAndLoadExtensions([], tempDir, tempDir);

		expect(result.errors).toHaveLength(0);
		expect(result.extensions).toHaveLength(1);
		expect(path.basename(result.extensions[0].path)).toBe("real.ts");
	});

	it("does not discover arbitrary *.ts under extensions/lib/ (use for shared imports only)", async () => {
		fs.writeFileSync(path.join(extensionsDir, "entry.ts"), extensionCode);
		const libDir = path.join(extensionsDir, "lib");
		fs.mkdirSync(libDir);
		fs.writeFileSync(path.join(libDir, "themeMap.ts"), `export const helper = 1;\n`);

		const result = await discoverAndLoadExtensions([], tempDir, tempDir);

		expect(result.errors).toHaveLength(0);
		expect(result.extensions).toHaveLength(1);
		expect(path.basename(result.extensions[0].path)).toBe("entry.ts");
	});
