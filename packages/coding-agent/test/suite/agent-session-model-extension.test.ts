import type { AgentTool, ThinkingLevel } from "@free/pi-agent-core";
import { fauxAssistantMessage, fauxToolCall, type Model } from "@free/pi-ai";
...
describe("AgentSession model and extension characterization", () => {
	...
	it("bindExtensions emits session_start and reload emits session_shutdown then session_start", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(pi) => {
					...
					pi.on("session_resources_ready", async (event) => {
						lifecycleEvents.push(`resources_ready:${event.reason}`);
					});
		...
		expect(lifecycleEvents).toEqual([
			"start:startup",
			"resources_ready:startup",
			"shutdown",
			"start:reload",
			"resources_ready:reload",
		]);
	});
});
