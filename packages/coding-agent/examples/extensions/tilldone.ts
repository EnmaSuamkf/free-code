...
import type { ExtensionAPI, ExtensionContext, Theme } from "@free/pi-coding-agent";
import { DynamicBorder } from "@free/pi-coding-agent";
import { Container, matchesKey, Text, truncateToWidth, visibleWidth } from "@free/pi-tui";
...
const TillDoneParams = Type.Object({
	...
	texts: Type.Optional(
		Type.Array(Type.String(), {
			description: "Multiple task texts (for add). Use this to batch-add several tasks at once.",
		}),
	),
...
class TillDoneListComponent {
	...
	render(width: number): string[] {
		...
		lines.push("");
		const heading = this.title ? th.fg("accent", ` ${this.title} `) : th.fg("accent", " TillDone ");
		...
		ctx.ui.setFooter((tui, theme, footerData) => {
			...
			return {
				...
				render(width: number): string[] {
					...
					const titleDisplay = listTitle ? theme.fg("accent", ` ${listTitle} `) : theme.fg("dim", " TillDone ");
