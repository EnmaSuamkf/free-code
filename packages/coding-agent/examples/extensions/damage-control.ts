...
export default function (pi: ExtensionAPI) {
	...
	let mcpSessionAuth: "unset" | "allowed" | "denied" = "unset";
			...
			if (isToolCallEventType("bash", event)) {
				...
				if (!violationReason) {
					...
					const READ_ONLY_COMMANDS =
						/^\s*(ls|cat|head|tail|grep|find|rg|wc|echo|stat|file|diff|less|more|sort|uniq|cut|awk|xargs\s+(?:ls|cat|head|tail|grep|find|rg|wc|echo|stat))\b/;
		...
		if (!violationReason && isRegisteredMcpTool(pi, event.toolName)) {
			...
			if (!ctx.hasUI) {
				pi.appendEntry("damage-control-log", {
					tool: event.toolName,
					input: event.input,
					rule: "mcp_requires_interactive_ui",
					action: "blocked",
				});
				ctx.abort();
				return {
					block: true,
					reason:
						"BLOCKED by Damage-Control: MCP tools require an interactive session so you can authorize them once per session.",
				};
			}
