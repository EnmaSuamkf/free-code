/**
 * lib/themeMap.ts — Per-extension default theme assignments
 *
 * Lives under `extensions/lib/` so auto-discovery does not treat this file as an
 * extension entry point (only top-level `*.ts` and `index.ts` packages are loaded).
 *
 * Each extension calls applyExtensionDefaults(import.meta.url, ctx) in session_start.
 */

import type { ExtensionContext } from "@free/pi-coding-agent";
...
export const THEME_MAP: Record<string, string> = {
	...
	"theme-toggle": "light", // dark ↔ light for light terminal backgrounds
