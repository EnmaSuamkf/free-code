...
export default function (pi: ExtensionAPI) {
	...
	function buildSubagentSpawnArgs(state: SubState, prompt: string, ctx: any): string[] {
		...
		const toolsCsv = builtins.length > 0 ? builtins.join(",") : DEFAULT_SUBAGENT_BUILTIN_TOOLS;
		const skillPaths = pi.getLoadedSkillPaths();
		const extPaths = pi.getLoadedExtensionPaths().filter((p) => path.basename(p) !== "subagent-widget.ts");
