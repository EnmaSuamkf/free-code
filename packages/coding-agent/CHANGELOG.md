...
- Default `models.json` seed (`initDefaultModelsJson` in migrations): on first run, when `~/.free-code/agent/models.json` is absent, free-code writes a starter file with an `only` whitelist set to the default model of each login-capable (OAuth) provider (Anthropic, OpenAI Codex, Gemini CLI, Google Antigravity, GitHub Copilot, from `defaultModelPerProvider`). New installs get a short, curated `/model` list instead of the full factory catalog. Never overwrites an existing file. Documented in `docs/local-models-setup.md` and `web-docs/capabilities.html`.
