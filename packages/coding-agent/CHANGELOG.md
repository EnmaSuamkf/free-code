...
- RPC `RpcClient`: JSONL requests on stdin are serialized so concurrent awaits (e.g. `get_commands` after webview load and `get_session_stats` for the Session monitor) cannot interleave writes and mis-associate responses; `getSessionStats()` tolerates assistant rows without `usage` yet (mid-stream).
- `profiles.json`: the `default` profile is no longer overwritten with an empty preset on every read, so saved tools/theme/`activeModel`/etc. on `default` persist across sessions.

### Added

- Default `models.json` seed (`initDefaultModelsJson` in migrations): on first run, when `~/.free-code/agent/models.json` is absent, free-code writes a starter file with an `only` whitelist set to the most-used models (4 per provider) of each login-capable (OAuth) provider (Anthropic, OpenAI Codex, Gemini CLI, Google Antigravity, GitHub Copilot — see `LOGIN_PROVIDER_MODELS`). New installs get a short, curated `/model` list instead of the full factory catalog. Never overwrites an existing file. Documented in `docs/local-models-setup.md` and `web-docs/capabilities.html`.
- `models.json` `only` whitelist: an optional top-level `"only": [...]` curates which models appear in `/model` (and `--list-models`, RPC/plugin/desktop pickers), including built-in/factory models. Entries match by `provider/id`, `id`, or `name` (case-insensitive, `.`/`-`/`_`/space treated alike) or as a glob (e.g. `lmstudio/**`, `anthropic/*sonnet*`). Empty/absent shows everything; if the whitelist matches nothing it fails open to the full list. Explicit `--model` selection still resolves via the full catalog. Filtering is applied once in `ModelRegistry.getAvailable()`.
- Bundled `default-extensions/lmstudio.ts`: registers a local LM Studio server as an OpenAI-compatible (`openai-completions`) provider, auto-discovering the currently loaded models via `GET /v1/models` (embedding models filtered out). Only registers when the server responds, so it's silent when LM Studio isn't running. Override the endpoint with `LMSTUDIO_BASE_URL`. Pick models with `/model`.
- `loadAgentEnvFile()` (config): on session creation, variables from `<agentDir>/.env` are loaded into `process.env` (gap-fill — existing/exported values always win). This makes `models.json` the single user-editable place to manage cloud provider models: keep credentials in `~/.free-code/agent/.env` and reference them by name (e.g. `"apiKey": "FIREWORKS_API_KEY"`), and both `/model` availability checks and requests resolve them. Replaces the ad-hoc `fireworks.ts` provider extension (removed) — add Fireworks/other cloud models in `models.json` instead. LM Studio keeps its auto-discovery extension.
- `mcp-client.ts` stdio servers now surface authentication URLs and inherit `.env`: the MCP subprocess's stderr is forwarded to the UI (login URLs from OAuth servers like `mcp-remote` were previously swallowed by the piped-but-unread stderr, so the user saw nothing to click), and env vars from `~/.free-code/agent/.env` + project `<cwd>/.free-code/.env` are injected into the subprocess (local overrides global, explicit `mcp.json` `env` wins). This makes both forms work: Docker/stdio servers reading credentials from `.env` (e.g. `-e VAR` forwarding) and Cloud servers needing OAuth.
- Bundled `default-extensions/mcp-command.ts`: `/mcp` (`list`, `enable <name>`, `disable <name>`) controls which MCP servers start automatically. Activation state lives in `~/.free-code/agent/mcp-status.json` (separate from `mcp.json`), managed by the shared `default-extensions/lib/mcp-status.ts`. On every session start the status is reconciled against `mcp.json`: newly configured servers default to **disabled** (adding a server by hand no longer auto-starts it) and removed servers are pruned. `mcp-client.ts` now only connects enabled servers. RPC `get_commands` advertises `mcp`.
- `getDefaultSkillsDir()` (config) and `syncDefaultSkills()` (migrations): bundled skills are now copied into `~/.free-code/agent/skills/` on every startup with force-overwrite, the same policy and code shape as `syncDefaultExtensions()`. Updated bundled skills (e.g. `gemini-browser`) reach the runtime on the next agent start without a manual copy or reinstall; user-authored skills with other names are preserved (the copy does not prune the target). This targets the directory `loadSkills()` actually scans (`getAgentDir()/skills`), unlike the install-time-only copy into `~/.free-code/skills/`.
- RAG: optional `*.knowledge.md` sidecar metadata (e.g. `<stem>.knowledge.md` next to a primary); `GET /discover?kb=...` on the local server; `/rag-kb use` and profile KB selection inject discover text when supported; `GET /query` excludes metadata chunks and legacy sidecar-shaped chunks. TUI/VS Code `addFile`/`addGroup` accept `.txt` alongside PDF/Word/Markdown.
- Bundled `default-extensions/profile-manager.ts`: `/profile` (`list`, `use`, `create`, `save`, `delete`, or interactive) and persistent session presets in `~/.free-code/agent/profiles.json` (tools, skills, agents, theme, and optional `activeModel`); mandatory `default` profile; startup profile picker when a UI is available (replaces separate startup wizards).
- `getProfilesPath()` and `src/core/user-profiles.ts` helpers (`readUserProfilesFile`, `writeUserProfilesFile`, `applyUserProfile`, `captureUserProfile`, `parseProfileArgs`, etc.) for tests and integrations.
- RPC `get_commands` and VS Code webview slash allowlist include `profile`.
- Bundled `default-extensions/browse-command.ts`: `/browse [url] [goal]` mirrors the VS Code plugin visible-browser flow (`agent_browser` headed open + snapshot); if the URL is omitted, `https://www.google.com` is used.
- RPC `get_commands` advertise `browse` alongside other bundled slash commands.
- **Vision Mode** (`default-extensions/vision.ts` and `lib/vision/`): multimodal extension with screen capture, speech-to-text (STT), text-to-speech (TTS), and hands-free mode. Three slash commands: `/see <question>` (capture screen + ask), `/voice [context]` (record audio + transcribe + ask), `/vision live [on|off]` (hands-free continuous listening with wake word detection). Push-to-talk shortcut (`Ctrl+Alt+V` by default, configurable via `/vision config shortcut <key>`). Wake word detection (default `"max"`, supports comma-separated alternatives, or empty for always-on processing). Screen capture backends: XDG Desktop Portal (Wayland), `gnome-screenshot`, `grim`, `spectacle`, `scrot`, `import` (ImageMagick X11), `screencapture` (macOS), PowerShell (Windows). Audio recorders: `pw-record` (PipeWire), `arecord` (ALSA), `sox`, `ffmpeg`. STT backends: OpenAI Whisper API (`whisper-1`), Groq Whisper API (`whisper-large-v3-turbo` / `whisper-large-v3` — 10-20x faster, free tier), `whisper-cpp` (local, offline). TTS backends: OpenAI TTS API (`tts-1` / `tts-1-hd`, voices: alloy/echo/fable/onyx/nova/shimmer), `espeak` (Linux local), `say` (macOS local). Config at `~/.free-code/agent/vision.json` or via `/vision config <key> [value]` and env vars (`OPENAI_API_KEY`, `GROQ_API_KEY`, `VISION_*`). Live mode features: keyword-based screen capture (only captures when utterance contains vision keywords like "qué ves", "mira", "what do you see", or when `liveAlwaysCapture: true`), TTS for assistant responses (interruptible via PTT shortcut), continuous listening loop (auto-restarts after each agent response). `/vision status` and `/vision backends` for diagnostics. Exported `resizeImage()` and `loadPhoton()` from `src/index.ts` for extension reuse. See `analysis/vision-mode-plan.md` for design, `README.md` Vision Mode section, and `web-docs/vision-mode.html` for complete setup and usage.
- RPC `get_commands` and the VS Code/macOS webview slash-menu allowlist (`host.mjs` `RPC_WEBVIEW_SLASH_ALLOWLIST`) now include `vision`, `see`, and `voice`. Previously these commands ran fine when typed (the RPC `prompt` dispatch never filtered them), but were invisible to slash-menu autocomplete and the `/commands` catalog in the VS Code plugin and the free-code-macos app, since both go through the same RPC bridge and only advertise allowlisted names.

### Changed

- New sessions enable built-in `grep`, `find`, and `ls` by default alongside `read`, `bash`, `edit`, and `write` (`DEFAULT_ACTIVE_BUILTIN_TOOL_NAMES` in `src/core/tools/index.js`; CLI `--tools` help and README updated). The default system prompt therefore prefers those tools over bash for exploration; bash-only sessions still get guidance to minimize long `find|grep|head` chains.
- User profiles (`profiles.json`): each profile may include `activeModel` (`provider` + `id`); `/profile save` captures the current session model and applying a profile calls `setModel` when that field is set (missing model or credentials yields a warning, session keeps prior model). `applyUserProfile` is now `async` because it awaits `setModel`.
- `default-extensions/browser` `shouldAppendBrowserSystemPrompt`: prompts that are only the VS Code/Cursor CDP attach recipe (`Execute agent_browser with exactly these params:` plus JSON containing `"--cdp"`) no longer trigger the extra `before_agent_start` browser appendix, since the turn already embeds the exact `agent_browser` invocation.
- `/session` now reports token `Total` as input plus output only and omits cache read/write rows from the displayed token summary.
- Global npm installs copy bundled skills into `~/.free-code/skills/` and bundled MCP files from `packages/coding-agent/mcps/` into `~/.free-code/agent/` (e.g. `mcp.json`) only when the destination path does not already exist — existing skill folders, `mcp.json`, `.env`, or other bundled names are never overwritten (avoids `ERR_FS_CP_DIR_TO_NON_DIR` when an old file collides with a bundled skill directory). If `mcps/example.env` is bundled, it is copied **only** to `~/.free-code/agent/.env` when that file is missing (secrets template); it is not installed as `example.env` beside other MCP files. Package builds sync skills from the repo-level `.free-code/skills/` directory and MCP assets from the repo-level `mcps/` directory into those package folders first (a real `mcps/.env` is never synced into the npm package — keep secrets local).
...
- `default-extensions/damage-control.ts` registers `/damage-control [on|off|toggle|status]` to enable or skip enforcement for the current session (rules stay loaded; new session resets to on).
- `default-extensions/damage-control.ts` loads `damage-control-rules.yaml` next to the extension module first, then falls back via `getBundledDamageControlRulesPath()` from `@free/pi-coding-agent` (uses the same `getDefaultExtensionsDir()` as the CLI, so it works when the extension lives under `agent/extensions` only). `npm run build` copies all of `default-extensions/` into `dist/default-extensions/`.
- `@free/jiti`, `@free/mini-lit`, and the `@free/clipboard` entrypoint are vendored as workspace packages under `packages/free-*` (lockstep version with the rest of the monorepo). Root and package manifests no longer use `npm:@mariozechner/*` dependency aliases; clipboard still pulls optional native bindings from npm under the upstream package names.
- Bundled `default-extensions/minimal.ts` footer: third line matches the tool-counter layout (model + colored context bar + percent on the left; accumulated tokens in/out and session cost on the right), with `onBranchChange` to refresh usage; a fourth line shows `ctx.ui.setStatus()` texts (e.g. damage-control) like the built-in footer.
- `ExtensionAPI` adds `getActiveBuiltinToolNames()`, `getLoadedSkillPaths()`, and `getLoadedExtensionPaths()` for subprocesses that should mirror the current session. The bundled `subagent-widget` uses these so JSON-mode subagents inherit the main session's built-in tools, skills, and extensions (excluding `subagent-widget.ts` itself to avoid nested `/sub`).
- Interactive startup and session switches show a bordered loading state in the editor (same component as `/reload`) while extensions and resources (skills, prompts, MCP tools, etc.) bind.
- The bundled `resource-picker` extension applies `/pick-tools`, `/pick-skill`, and `/pick-agent` on demand only (no automatic startup pickers); optional resources are chosen via `/profile` at startup when a UI is present.
- Bundled `questionnaire` and `subagent-widget` extension tools are always active and hidden from the optional tool picker, so they cannot be disabled from the CLI or RPC picker.
- Bundled `subagent-widget` now sends plain `setWidget` lines when running under RPC, allowing IDE hosts to render live subagent status instead of dropping the TUI-only widget factory.
- Bundled `subagent-widget` RPC updates now include an activity timeline with thinking deltas, text progress, tool start/end events, and process completion so IDE hosts can render subagent tabs with live progress details.
- In RPC, bundled `subagent-widget` now stores completed subagent results as `nextTurn` context instead of triggering an immediate follow-up model turn, so the host can display the result while the next user prompt can still use it as context. Interactive TUI keeps the existing visible follow-up behavior.
- Bundled `subagent-widget` now gives each subagent widget a unique execution key and keeps the visible counter in process-global state, preventing late results from an older `Subagent #1` from updating a newer subagent card after RPC extension rebinding.
- Bundled `subagent-widget` subagents now inherit the parent session's active tool set, including active MCP tools. A small `subagent-tool-inheritance` helper applies the active tool list after child extensions/MCP resources finish loading, and `damage-control` accepts MCP authorization inherited by the spawned subagent process.
- Extension loader no longer registers alternate scoped imports for `pi-*` packages; extension source must use `@free/pi-coding-agent`, `@free/pi-ai`, `@free/pi-tui`, and `@free/pi-agent-core` (bundled for jiti and the compiled binary).
- Default model for `google-vertex` is now `gemini-2.5-pro` instead of `gemini-3-pro-preview`, since preview IDs are not available in every GCP project or region.
- Rebranded the CLI binary and `piConfig` defaults to `free-code` with config under `~/.free-code/agent`. Extension `package.json` manifests may use the app key `free-code` or legacy `pi`. `getAgentDir()` still honors `PI_CODING_AGENT_DIR` when the new `FREE_CODE_CODING_AGENT_DIR` is unset.

### Fixed

- `set_active_profile` (and similar tools that change tools, system prompt, or model mid-turn): the next LLM call in the **same** user message still used the snapshot from turn start, so the profile looked applied in the tool result but the following model step ignored it. `@free/pi-agent-core` now refreshes loop context from live agent state before each provider request.
- `default-extensions/gemini-command.ts`: `/gemini ask` and `/gemini download` embed skill bodies from `~/.free-code/agent/skills/...` via `getAgentDir()` instead of `<cwd>/.free-code/skills/...`.
- Bundled `set-active-model` extension: `set_active_model` now ships `promptSnippet` and `promptGuidelines`, and tool/parameter descriptions no longer tell the model to pre-check names against an "available-models list" from memory. That wording led some models to refuse switching (e.g. to newer Copilot ids) without calling the tool; validation remains server-side in `execute`.
- `default-extensions/mcp-client.ts`: stdio MCP servers now expand `~/`, `$HOME`, and `${HOME}` in `command`/`args` (spawn does not use a shell, so those tokens used to be passed through literally). The subprocess working directory is set to `~/.free-code/agent`, so `--env-file ./.env` can point at the `.env` next to your global `mcp.json` without an absolute path.
- `default-extensions/rag-manager.ts` (and example copy): `/rag add` path normalization trims whitespace, strips `[path]` wrappers, removes nested matching quotes (ASCII and typographic), then stray leading/trailing `'`/`"` so `extname` sees `.pdf`/`.md` instead of `.pdf'`/`.md'`.
- `default-extensions/questionnaire.ts` now works in RPC mode (e.g. the VS Code plugin). Previously the tool short-circuited with `Error: UI not available (running in non-interactive mode)` because `ctx.hasUI === false` in RPC and the implementation only had a `ctx.ui.custom()` TUI path. It now prefers the new `ctx.ui.questionnaire` host method (rendered inline by the VS Code free-code plugin's chat panel) and gracefully falls back to chained `ctx.ui.select` (+ `ctx.ui.input` for the "Type something." option) dialogs when the host signals `unsupported: true`, so older RPC clients still get a working questionnaire.
- `default-extensions/damage-control.ts` no longer auto-blocks MCP tool calls in RPC mode. The previous `!ctx.hasUI` early-return aborted the first MCP call in embedders (e.g. the VS Code plugin) because `ctx.hasUI === false` in RPC, so the user never saw the "Allow MCP tools for this session?" prompt and the agent ended with the block reason as its answer. The authorization `confirm()` now runs unconditionally; it works in the TUI (native dialog), in RPC (extension UI sub-protocol — host shows the dialog), and in print/noOp mode (resolves to `false` instantly so the call is still denied). The previous `!ctx.hasUI` early-return aborted the first MCP call in embedders (e.g. the VS Code plugin) because `ctx.hasUI === false` in RPC, so the user never saw the "Allow MCP tools for this session?" prompt and the agent ended with the block reason as its answer. The authorization `confirm()` now runs unconditionally; it works in the TUI (native dialog), in RPC (extension UI sub-protocol — host shows the dialog), and in print/noOp mode (resolves to `false` instantly so the call is still denied).
- RPC `default` (unknown command): error responses now include the request `id`, so embedders that correlate by `id` no longer hang until timeout when the binary does not recognize a command (e.g. older `free-code` without newer RPC verbs).
- Bundled `subagent-widget` no longer clears its in-memory subagent list on repeated `session_start: startup` extension binds, preventing RPC hosts from reusing `Subagent #1` for a later subagent in the same chat.
- `ctx.ui.select()` with `onNavigate` (e.g. theme preview): list row colors now match the previewed theme on the same keypress. Previously `updateList()` ran before `onNavigate`, so `theme.fg("accent", …)` used the old palette until a second arrow press.
- Changing theme during a session now fully rebuilds the chat so all previously rendered content (messages, resource list, status lines) is re-emitted with the new palette. During streaming or compaction the rebuild is deferred to avoid dropping the in-progress component.
- Extension loader now exposes the `yaml` package to dynamically loaded extensions (user `agent/extensions` and default-extensions), fixing `Cannot find module 'yaml'` when an extension imports it (e.g. damage-control rules files).
- `subagent-widget` extension: background JSON-mode subprocess now invokes the same CLI as the parent process (`free-code` or `pi`) instead of hardcoding `pi` (fixes `spawn pi ENOENT` on rebranded installs). Override with `PI_SUBAGENT_CLI` or `FREE_CODE_SUBAGENT_CLI` if needed. Subagent session files use `getAgentDir()` so they land under the active agent dir (e.g. `~/.free-code/agent/sessions/subagents/`). Subagents inherit the main session thinking level (`getThinkingLevel()`) instead of always using `off`.
- Extension auto-discovery skips `agent/extensions/_*.ts` (and `.js`), and does not scan arbitrary `*.ts` inside subfolders (only `index.ts` / package manifests). Place shared modules such as `themeMap.ts` under `agent/extensions/lib/` and import `./lib/themeMap.ts` from extensions.
- Missing `-e` extension paths now suggest setting `FREE_CODE_ROOT` (or `{APP}_ROOT`) when unset, so running from another project directory is easier to diagnose.
...
- Markdown session export: `exportSessionToMarkdown()` / `AgentSession.exportToMarkdown()`, TUI `/export <path>.md`, **`free-code --export session.jsonl out.md`** (CLI sin TUI; segundo argumento `.md`), `exportMarkdownFromSessionFile()` for arbitrary `.jsonl` session files, and RPC `export_markdown` (includes thinking blocks, tool calls and results, compaction, model changes, custom entries, and labels on the current branch). VS Code extension: **Export** chat toolbar button and command **Free Code: Export conversation to Markdown** (uses RPC when the agent session is persisted; otherwise falls back to the sidebar transcript).
- `/rag add` and the local RAG indexer accept **Markdown** (`.md`) in addition to PDF and Word; `.md` files are loaded as UTF-8 text like `.txt` on the Python server.
- Bundled `set-active-model` extension registers **`get_active_model`** (no arguments): returns the session's active `provider`, `modelId`, and display name from `ctx.model`, so temporary `set_active_model` workflows (e.g. image comparison) can restore the prior model reliably.
- `ExtensionUIContext.questionnaire?` optional method for hosts that can render a rich, multi-question form (RPC sub-protocol method `questionnaire`). The bundled `questionnaire` tool prefers it over chained dialog fallbacks. Hosts that haven't implemented the method should reply with `{ cancelled: true, unsupported: true }` so callers can fall back gracefully. New types: `QuestionnaireDialogQuestion`, `QuestionnaireDialogOption`, `QuestionnaireDialogAnswer`, `QuestionnaireDialogResult` (re-exported from `@free/pi-coding-agent`).
- RPC commands `get_skill_picker_state` and `set_skill_picker` to toggle `<skill>` blocks in the merged system prompt (same behavior as `/pick-skill` / `default-extensions/resource-picker.ts`), backed by `src/core/skill-picker-state.ts`. `ExtensionAPI` adds `getResourceDisplayFilter()` so extensions and the host stay aligned with `setResourceDisplayFilter`.
- RPC commands `get_tool_picker_state` and `set_tool_picker` for optional MCP/extension tool groups (same behavior as TUI `/pick-tools`), backed by `src/core/tool-picker-groups.ts`.
- RPC `prompt` with `source: "rpc"`: message exactly `/session` emits a `session_info` `AgentSessionEvent` (session file, id, optional name, message counts, tokens, cost, context usage) instead of calling the model, matching interactive `/session` data for IDE embedders.
- Added a stable `@free/pi-coding-agent/rpc-client` subpath export for `RpcClient` and RPC command/response types, so external integrations can consume the RPC client API without relying on internal `dist` paths.
- `ExtensionUIDialogOptions` gains `onNavigate?: (option: string) => void`: called on each arrow-key move in `ctx.ui.select()`, enabling live previews.
- `default-extensions/resource-picker.ts`: added `/pick-skill` command to enable/disable skills in the system prompt on demand, matching `/pick-tools` and `/pick-agent`.
- `default-extensions/direct-bash.ts`: `/sh` now supports Tab completion for filesystem paths in command arguments. Completions resolve relative to the tracked shell directory (`currentDir`) so `cd Docu<Tab>` shows entries in the current shell directory.
- `default-extensions/startup-theme-picker.ts`: `/pick-theme` now previews each theme as you navigate the list and restores the original theme on cancel.
- Built-in TUI themes from [oh-pi](https://github.com/ifiokjr/oh-pi/tree/main/packages/themes/themes): `catppuccin-mocha`, `cyberpunk`, `gruvbox-dark`, `nord`, `oh-p-dark`, `tokyo-night` (loaded with `dark` and `light` from `src/modes/interactive/theme/*.json`).
- Optional auto-start of the local Python RAG server on CLI startup: if `main.py` and `requirements.txt` are found (see `FREE_CODE_RAG_SERVER_DIR`, `~/.free-code/agent/rag-server-dir`, or a sibling `../free-code-rag` checkout next to the monorepo root), `free-code` runs `pip3 install -r requirements.txt` and `python3 main.py` when nothing is listening on the configured RAG URL yet. Disable with `--no-rag-server` or `FREE_CODE_RAG_SERVER_AUTO=0`. When auto-start runs, the CLI prints a dim `RAG: local server …` line on startup (yellow warning if the server does not become ready in time).
- Bundled RAG integration: `default-extensions/rag-manager.ts` registers `/rag` (`add`, `use`, `list`, `remove`), copies files to `~/.free-code/knowledgeBase/`, and calls the local RAG server (`http://localhost:8085`, overridable via `FREE_CODE_RAG_SERVER_URL`). Agent Skill `skills/rag/SKILL.md` documents the workflow for models and `/skill:rag`.
- Extension event `session_resources_ready`: emitted after `session_start` and after extension-discovered resources are merged, so handlers can use `ctx.getSystemPrompt()` and tools before the first user message.
- `FREE_CODE_ROOT` (or `{APP}_ROOT` for other branded builds): when set, relative `--extension` / `--skill` / `--prompt` / `--theme` paths that do not exist under the process cwd are resolved against this directory so `-e extensions/foo.ts` works without `cd` into the repo.
...
- Fixed monorepo `tsconfig.json` path mappings to resolve `@free/pi-ai` subpath exports to source files in development checkouts ([#2625](https://github.com/badlogic/pi-mono/pull/2625) by [@ferologics](https://github.com/ferologics))
...
- Added missing `ajv` direct dependency; previously relied on transitive install via `@free/pi-ai` which broke standalone installs ([#2252](https://github.com/badlogic/pi-mono/issues/2252))
...
- Built-in tools now work like custom tools in extensions. To get built-in tool definitions, import `readToolDefinition` / `createReadToolDefinition()` and the equivalent `bash`, `edit`, `write`, `grep`, `find`, and `ls` exports from `@free/pi-coding-agent`.
...
- Added `ToolCallEventResult` to the `@free/pi-coding-agent` top-level and core extension exports so extension authors can type explicit `tool_call` handler return values ([#2458](https://github.com/badlogic/pi-mono/issues/2458))
...
- Faster startup by lazy-loading `@free/pi-ai` provider SDKs on first use instead of import time ([#2297](https://github.com/badlogic/pi-mono/issues/2297))
...
- Lazy-load built-in `@free/pi-ai` provider modules and root provider wrappers so coding-agent startup no longer eagerly loads provider SDKs before first use ([#2297](https://github.com/badlogic/pi-mono/issues/2297))
...
- Fixed `@free/pi-ai/oauth` extension imports in published installs by resolving the subpath directly from built `dist` files instead of package-root wrapper shims ([#1856](https://github.com/badlogic/pi-mono/issues/1856)).
...
- Moved Node OAuth runtime exports off the top-level `@free/pi-ai` entry. OAuth login and refresh must be imported from `@free/pi-ai/oauth` ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
...
- Fixed extension loading with the new `@free/pi-ai/oauth` export path by aliasing the oauth subpath in the extension loader and development path mapping ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
...
- Fixed browser bundling compatibility for `@free/pi-ai` by removing Node-only side effects from default browser import paths ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
...
- Fixed extension loading in compiled Bun binary: extensions with local file imports now work correctly. Updated `@free/jiti` to v2.6.5 which bundles babel for Bun binary compatibility. ([#681](https://github.com/badlogic/pi-mono/issues/681))
...
- Extensions now load correctly in compiled Bun binary using `@free/jiti` fork with `virtualModules` support. Bundled packages (`@sinclair/typebox`, `@free/pi-tui`, `@free/pi-ai`, `@free/pi-coding-agent`) are accessible to extensions without filesystem node_modules.
...
```typescript
// Before (hook)
import type { HookAPI, HookContext } from "@free/pi-coding-agent";
export default function (pi: HookAPI) { ... }

// Before (custom tool)
import type { CustomToolFactory } from "@free/pi-coding-agent";
...
- `Attachment` type removed. Use `ImageContent` from `@free/pi-ai` instead. Add images directly to message content arrays.
...
```typescript
import { discoverAuthStorage, discoverModels } from "@free/pi-coding-agent";
...
- **SDK API changes** ([#296](https://github.com/badlogic/pi-mono/issues/296)):
  ...
  - Use `getModel()` from `@free/pi-ai` for built-in models, `modelRegistry.find()` for custom models + built-in models
...
- **OAuth and model config exports**: Scripts using `AgentSession` directly can now import `getAvailableModels`, `getApiKeyForModel`, `findModel`, `login`, `logout`, and `getOAuthProviders` from `@free/pi-coding-agent` to reuse OAuth token storage and model resolution. ([#245](https://github.com/badlogic/pi-mono/issues/245))
...
- **`getMarkdownTheme()` export**: Custom tools can now import `getMarkdownTheme()` from `@free/pi-coding-agent` to use the same markdown styling as the main UI.
...
- Hooks loader now supports same import aliases as custom tools (`@sinclair/typebox`, `@free/pi-ai`, `@free/pi-tui`, `@free/pi-coding-agent`).
...
- Fixed custom tools failing to load from `~/.pi/agent/tools/` when pi is installed globally. Module imports (`@sinclair/typebox`, `@free/pi-tui`, `@free/pi-ai`) are now resolved via aliases.
...
- Updated `@free/pi-ai` with X-Initiator header support for GitHub Copilot, ensuring agent calls are not deducted from quota. ([#200](https://github.com/badlogic/pi-mono/pull/200) by [@kim0](https://github.com/kim0))
...
- Updated `@free/pi-ai` with interleaved thinking enabled by default for Anthropic Claude 4 models.
...
- Updated `@free/pi-ai` with interleaved thinking support for Anthropic models.
