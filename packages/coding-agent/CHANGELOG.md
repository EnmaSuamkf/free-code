...
- Bundled `default-extensions/fireworks.ts`: registers Fireworks AI as an OpenAI-compatible (`openai-completions`) provider when a `FIREWORKS_API_KEY` is configured (read from env or `~/.free-code/agent/.env`); stays silent otherwise. Models are configurable via `FIREWORKS_MODELS` (comma/space-separated ids), defaulting to the Kimi K2.7 Code serverless model; `FIREWORKS_BASE_URL` overrides the endpoint. Pick registered models with `/model`.
