...
> **Placement for /reload:** Put extensions in `~/.free-code/agent/extensions/` (global) or `.free-code/agent/extensions/` (project-local) for auto-discovery. Use `free-code -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.

**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `free-code.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `free-code.registerCommand()`
- **Session persistence** - Store state that survives restarts via `free-code.appendEntry()`
...
Create `~/.free-code/agent/extensions/my-extension.ts`:
...
| Location | Scope |
|----------|-------|
| `~/.free-code/agent/extensions/*.ts` | Global (all projects) |
| `~/.free-code/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.free-code/agent/extensions/*.ts` | Project-local |
| `.free-code/agent/extensions/*/index.ts` | Project-local (subdirectory) |
...
| Package | Purpose |
|---------|---------|
| `@free/pi-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `@sinclair/typebox` | Schema definitions for tool parameters |
| `@free/pi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@free/pi-tui` | TUI components for custom rendering |
...
Fired after `session_start` and after optional `resources_discover` paths have been merged into the resource loader. The effective system prompt (including skills) and tool registry are up to date; use this for startup UI that must run before the first message.
...
```typescript
import { isToolCallEventType } from "@free/pi-coding-agent";
...
```typescript
import { isBashToolResult } from "@free/pi-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@free/pi-coding-agent";
...
```typescript
import { SessionManager } from "@free/pi-coding-agent";
...
```typescript
import { Type } from "@sinclair/typebox";
import { StringEnum } from "@free/pi-ai";
...
```typescript
import type { AutocompleteItem } from "@free/pi-tui";
...
```typescript
import { withFileMutationQueue } from "@free/pi-coding-agent";
...
**Important:** Use `StringEnum` from `@free/pi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@free/pi-coding-agent";
...
```typescript
import { createBashTool } from "@free/pi-coding-agent";
...
```typescript
import { keyHint } from "@free/pi-coding-agent";
...
```typescript
import { Text, Component } from "@free/pi-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@free/pi-coding-agent";
import { matchesKey } from "@free/pi-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@free/pi-coding-agent";
