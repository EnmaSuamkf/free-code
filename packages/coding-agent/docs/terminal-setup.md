...
Add to `keybindings.json` to enable `Shift+Enter` for multi-line input (the sequence is the [Kitty `CSI u`](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) form for **Shift+Enter** on the main Enter key):
...
**Wrong `args.text`:** Do not set `text` to `"\u001b\r"` (ESC + carriage return). That is not the Kitty **CSI u** encoding of Shift+Enter; the TUI will not treat it as `tui.input.newLine` in a reliable way. The correct value is **`"\u001b[13;2u"`** as above.
