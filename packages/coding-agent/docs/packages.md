...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@free/pi-ai`, `@free/pi-agent-core`, `@free/pi-coding-agent`, `@free/pi-tui`, `@sinclair/typebox`.
