...
```typescript
import { AuthStorage, createAgentSession, ModelRegistry, SessionManager } from "@free/pi-coding-agent";
...
```typescript
import { createAgentSession } from "@free/pi-coding-agent";
...
The `Agent` class (from `@free/pi-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
import { AuthStorage, ModelRegistry } from "@free/pi-coding-agent";
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@free/pi-coding-agent";
...
```typescript
import {
  codingTools,   // read, bash, edit, write (subset for lightweight SDK use)
  readOnlyTools, // read, grep, find, ls
  readTool, bashTool, editTool, writeTool,
  grepTool, findTool, lsTool,
} from "@free/pi-coding-agent";

// Omit `tools` to get defaults: read, bash, edit, write, grep, find, ls

const { session: readOnlySession } = await createAgentSession({
  tools: readOnlyTools,
});

const { session: pickedSession } = await createAgentSession({
...
- If you omit `tools`, pi uses the default built-ins (`read`, `bash`, `edit`, `write`, `grep`, `find`, `ls`) with the correct `cwd`
...
import { createAgentSession, defineTool } from "@free/pi-coding-agent";
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@free/pi-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@free/pi-coding-agent";
