...
export class FreeCodeChatViewProvider {
   ...
   *
   * macOS only for now: uses the bundled Google Chrome.app binary path.
   */
  async handleLaunchChromeDebug() {
    const port = CHROME_DEBUG_PORT;
    if (process.platform !== "darwin") {
      const msg = `Launching Chrome with remote debugging is currently only wired up for macOS (platform=${process.platform}).`;
    ...
    if (!alreadyListening) {
      // Run through bash so `$HOME` expands and escaped spaces in the .app path are honored.
      const cmd =
