...
function appNameToEnvPrefix(name) {
	const upper = name.toUpperCase().replace(/[^A-Z0-9]+/g, "_");
	const trimmed = upper.replace(/^_+|_+$/g, "");
	return trimmed.length > 0 ? trimmed : "PI";
}

const codingAgentPkg = JSON.parse(readFileSync(join(packageDir, "package.json"), "utf-8"));
const agentDirEnvName = `${appNameToEnvPrefix(codingAgentPkg.piConfig?.name || "pi")}_CODING_AGENT_DIR`;
